...
class AdminAuth extends Controller
{
    ...
    public function reset_password_send($token, Request $request)
    {
        $check_token = DB::table('password_resets')->where('token', $token)
            ->where('created_at', '>', Carbon::now()->subHours(2))->first();
        if (!empty($check_token) && request('password') === request('password_confirm')) {
            $admin = Admin::where('email', $check_token->email)->update([
                'email' => $check_token->email,
                'password' => bcrypt(request('password'))]);
            DB::table('password_resets')->where('email', request('email'))->delete();
            session()->flash('success_updated', trans('admin.updated_password'));
            return redirect(aurl('login'));
        } else {
            session()->flash('error', trans('admin.error_match'));
            return redirect(aurl('reset/password/' . $token));
        }
